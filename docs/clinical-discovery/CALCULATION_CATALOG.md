...
|---|---|---|---|---|
| `uterineArtery.meanPi` | PI médio das artérias uterinas | `(PI direita + PI esquerda) / 2` | `SOURCE_NOT_REQUIRED` (definição aritmética) | `APPROVED` — pronto para implementação como cálculo puro |
| `fetus.cpr` | Relação cerebroplacentária | `PI da ACM / PI da artéria umbilical` | fórmula aprovada; **interpretação** por IG `SOURCE_VALIDATION_PENDING` | `SOURCED` na fórmula, bloqueado na classificação |
| `fetus.efw.hadlock` | Peso fetal estimado (Hadlock) | conceito aprovado; fórmula/versão exatas pendentes | `SOURCE_VALIDATION_PENDING` | `NAMED` |
| `fetus.efwPercentile` | Percentil de peso fetal | depende de tabela por IG | `SOURCE_VALIDATION_PENDING` | `NAMED` |
| `fetus.gestationalAgeByBiometry` | IG estimada geral pela biometria | composição das medidas; método exato pendente | `SOURCE_VALIDATION_PENDING` | `NAMED` |
| `pregnancy.correctedGestationalAge` | IG corrigida (datação) | política de datação + aritmética de semanas/dias | `SOURCE_VALIDATION_PENDING` (política e interpolação) | `NAMED` |
...
- RCP **não** usa corte fixo `< 1`; a classificação depende de idade gestacional precisa.
...
- Cálculo puramente aritmético e definicional (`uterineArtery.meanPi`) pode ser implementado sem referência externa, mas a **comparação com P95** não.
